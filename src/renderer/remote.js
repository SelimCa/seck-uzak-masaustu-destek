(function bootstrapRemote() {
  const query = new URLSearchParams(window.location.search);

  const DEVICE_BOOK_KEY = 'seck-device-book-v1';

  const config = {
    targetCode: String(query.get('targetCode') || '').trim(),
    credential: String(query.get('credential') || '').trim(),
    viewerName: String(query.get('viewerName') || 'Seck Kullanici').trim(),
    signalServerUrl: String(query.get('signalServerUrl') || 'http://127.0.0.1:3131').trim(),
    turnServerUrl: String(query.get('turnServerUrl') || '').trim(),
    turnUsername: String(query.get('turnUsername') || '').trim(),
    turnPassword: String(query.get('turnPassword') || '').trim(),
  };

  const refs = {
    statusText: document.querySelector('#statusText'),
    sessionBadge: document.querySelector('#sessionBadge'),
    fullscreenButton: document.querySelector('#fullscreenButton'),
    disconnectButton: document.querySelector('#disconnectButton'),
    remoteSurface: document.querySelector('#remoteSurface'),
    remoteVideo: document.querySelector('#remoteVideo'),
    overlay: document.querySelector('#overlay'),
  };

  const state = {
    socket: null,
    peerConnection: null,
    dataChannel: null,
    hostSocketId: null,
    controlActive: false,
    lastMoveAt: 0,
    hostName: '',
    thumbTimer: null,
  };

  function setStatus(text) {
    refs.statusText.textContent = text;
  }

  function setBadge(type, text) {
    refs.sessionBadge.className = `badge ${type}`;
    refs.sessionBadge.textContent = text;
  }

  function normalizeCode(input) {
    return String(input || '').replace(/[^0-9]/g, '');
  }

  function formatCode(input) {
    const compact = normalizeCode(input);
    if (compact.length !== 9) {
      return input;
    }

    return `${compact.slice(0, 3)}-${compact.slice(3, 6)}-${compact.slice(6, 9)}`;
  }

  function loadBook() {
    try {
      const parsed = JSON.parse(localStorage.getItem(DEVICE_BOOK_KEY) || '{}');
      return {
        devices: parsed.devices && typeof parsed.devices === 'object' ? parsed.devices : {},
      };
    }
    catch {
      return { devices: {} };
    }
  }

  function saveBook(book) {
    localStorage.setItem(DEVICE_BOOK_KEY, JSON.stringify(book));
  }

  function saveDeviceSnapshot(thumbnailDataUrl) {
    const code = formatCode(config.targetCode);
    if (normalizeCode(code).length !== 9) {
      return;
    }

    const book = loadBook();
    const current = book.devices[code] || {
      code,
      alias: '',
      favorite: false,
    };

    book.devices[code] = {
      ...current,
      code,
      lastKnownName: state.hostName || current.lastKnownName || '',
      lastConnectedAt: Date.now(),
      thumbnailDataUrl: thumbnailDataUrl || current.thumbnailDataUrl || '',
    };

    saveBook(book);
  }

  function captureThumbnail() {
    if (!refs.remoteVideo.videoWidth || !refs.remoteVideo.videoHeight) {
      return;
    }

    const canvas = document.createElement('canvas');
    canvas.width = refs.remoteVideo.videoWidth;
    canvas.height = refs.remoteVideo.videoHeight;
    const context = canvas.getContext('2d');
    context.drawImage(refs.remoteVideo, 0, 0, canvas.width, canvas.height);
    const dataUrl = canvas.toDataURL('image/jpeg', 0.7);
    saveDeviceSnapshot(dataUrl);
  }

  function buildIceServers() {
    const servers = [{ urls: 'stun:stun.l.google.com:19302' }];
    if (config.turnServerUrl) {
      servers.push({
        urls: config.turnServerUrl,
        username: config.turnUsername,
        credential: config.turnPassword,
      });
    }
    return servers;
  }

  function sendJson(message) {
    if (state.dataChannel?.readyState === 'open') {
      state.dataChannel.send(JSON.stringify(message));
    }
  }

  function sendInput(payload) {
    sendJson({ kind: 'input', payload });
  }

  function showOverlay(text) {
    refs.overlay.textContent = text;
    refs.overlay.classList.remove('hidden');
  }

  function hideOverlay() {
    refs.overlay.classList.add('hidden');
  }

  function pointerToNormalized(event) {
    const rect = refs.remoteVideo.getBoundingClientRect();
    const videoWidth = refs.remoteVideo.videoWidth || rect.width;
    const videoHeight = refs.remoteVideo.videoHeight || rect.height;

    if (!rect.width || !rect.height || !videoWidth || !videoHeight) {
      return { xNorm: 0, yNorm: 0 };
    }

    const videoAspect = videoWidth / videoHeight;
    const boxAspect = rect.width / rect.height;

    let renderedWidth = rect.width;
    let renderedHeight = rect.height;
    let offsetX = 0;
    let offsetY = 0;

    if (boxAspect > videoAspect) {
      renderedWidth = rect.height * videoAspect;
      offsetX = (rect.width - renderedWidth) / 2;
    }
    else {
      renderedHeight = rect.width / videoAspect;
      offsetY = (rect.height - renderedHeight) / 2;
    }

    const localX = Math.min(Math.max(event.clientX - rect.left - offsetX, 0), renderedWidth);
    const localY = Math.min(Math.max(event.clientY - rect.top - offsetY, 0), renderedHeight);

    return {
      xNorm: renderedWidth ? localX / renderedWidth : 0,
      yNorm: renderedHeight ? localY / renderedHeight : 0,
    };
  }

  function activateControl() {
    state.controlActive = true;
    refs.remoteSurface.classList.add('active');
    refs.remoteSurface.focus();
  }

  function deactivateControl() {
    state.controlActive = false;
    refs.remoteSurface.classList.remove('active');
  }

  function installControlListeners() {
    refs.remoteSurface.addEventListener('click', () => activateControl());

    refs.remoteSurface.addEventListener('mousedown', (event) => {
      activateControl();
      const pointer = pointerToNormalized(event);
      const button = event.button === 2 ? 'right' : event.button === 1 ? 'middle' : 'left';
      sendInput({ type: 'click', button, ...pointer });
    });

    refs.remoteSurface.addEventListener('dblclick', (event) => {
      const pointer = pointerToNormalized(event);
      sendInput({ type: 'doubleClick', button: 'left', ...pointer });
    });

    refs.remoteSurface.addEventListener('mousemove', (event) => {
      if (!state.controlActive) {
        return;
      }

      const now = Date.now();
      if (now - state.lastMoveAt < 30) {
        return;
      }

      state.lastMoveAt = now;
      sendInput({ type: 'move', ...pointerToNormalized(event) });
    });

    refs.remoteSurface.addEventListener('wheel', (event) => {
      if (!state.controlActive) {
        return;
      }

      event.preventDefault();
      sendInput({ type: 'scroll', deltaY: event.deltaY < 0 ? 120 : -120 });
    }, { passive: false });

    refs.remoteSurface.addEventListener('contextmenu', (event) => {
      event.preventDefault();
    });

    refs.remoteSurface.addEventListener('blur', () => {
      deactivateControl();
    });

    window.addEventListener('keydown', (event) => {
      if (!state.controlActive) {
        return;
      }

      if (document.activeElement && ['INPUT', 'TEXTAREA'].includes(document.activeElement.tagName)) {
        return;
      }

      event.preventDefault();
      if (event.key.length === 1 && !event.ctrlKey && !event.altKey && !event.metaKey) {
        sendInput({ type: 'text', text: event.key });
        return;
      }

      sendInput({
        type: 'key',
        key: event.key === ' ' ? 'Space' : event.key,
        ctrl: event.ctrlKey,
        alt: event.altKey,
        shift: event.shiftKey,
        meta: event.metaKey,
      });
    });
  }

  function setupDataChannel(channel) {
    state.dataChannel = channel;
    channel.onopen = () => {
      setStatus('Baglandi. Uzak ekrani kendi bilgisayarin gibi kullanabilirsin.');
      hideOverlay();
      setBadge('ok', 'Bagli');
      saveDeviceSnapshot('');
    };

    channel.onclose = () => {
      setStatus('Veri kanali kapandi.');
      setBadge('danger', 'Baglanti kapandi');
    };
  }

  async function createViewerPeer() {
    state.peerConnection = new RTCPeerConnection({ iceServers: buildIceServers() });

    state.peerConnection.onicecandidate = ({ candidate }) => {
      if (!candidate || !state.hostSocketId) {
        return;
      }

      state.socket.emit('signal:relay', {
        to: state.hostSocketId,
        payload: { candidate },
      });
    };

    state.peerConnection.onconnectionstatechange = () => {
      const currentState = state.peerConnection?.connectionState;
      if (['disconnected', 'failed', 'closed'].includes(currentState)) {
        showOverlay('Baglanti koptu');
        setBadge('danger', 'Baglanti koptu');
      }
    };

    state.peerConnection.ontrack = (event) => {
      const [stream] = event.streams;
      refs.remoteVideo.srcObject = stream;
      refs.remoteVideo.play().catch(() => {});
      hideOverlay();

      if (!state.thumbTimer) {
        state.thumbTimer = setInterval(captureThumbnail, 5000);
      }
    };

    const channel = state.peerConnection.createDataChannel('anydeks-control');
    setupDataChannel(channel);

    const offer = await state.peerConnection.createOffer({ offerToReceiveVideo: true, offerToReceiveAudio: true });
    await state.peerConnection.setLocalDescription(offer);

    state.socket.emit('signal:relay', {
      to: state.hostSocketId,
      payload: { description: state.peerConnection.localDescription },
    });
  }

  function installTopbarActions() {
    refs.fullscreenButton.addEventListener('click', async () => {
      if (!document.fullscreenElement) {
        if (refs.remoteSurface.requestFullscreen) {
          await refs.remoteSurface.requestFullscreen();
        }
        else if (refs.remoteSurface.webkitRequestFullscreen) {
          refs.remoteSurface.webkitRequestFullscreen();
        }
      }
      else {
        if (document.exitFullscreen) {
          await document.exitFullscreen();
        }
        else if (document.webkitExitFullscreen) {
          document.webkitExitFullscreen();
        }
      }
    });

    refs.disconnectButton.addEventListener('click', () => {
      if (state.socket && state.hostSocketId) {
        state.socket.emit('session:end', {
          to: state.hostSocketId,
          reason: 'Izleyici oturumu sonlandirdi.',
        });
      }

      if (state.peerConnection) {
        state.peerConnection.close();
        state.peerConnection = null;
      }
      if (state.socket) {
        state.socket.disconnect();
      }
      window.close();
    });
  }

  function connect() {
    if (!config.targetCode || !config.credential) {
      refs.overlay.textContent = 'Hedef kod veya sifre eksik.';
      setBadge('danger', 'Eksik bilgi');
      return;
    }

    state.socket = window.io(config.signalServerUrl, {
      transports: ['websocket'],
      reconnection: true,
    });

    state.socket.on('connect', () => {
      setStatus('Baglanti istegi gonderiliyor...');
      state.socket.emit('session:request', {
        targetCode: formatCode(config.targetCode),
        viewerName: config.viewerName,
        credential: config.credential,
      });
    });

    state.socket.on('session:accepted', async ({ hostSocketId, hostInfo }) => {
      state.hostSocketId = hostSocketId;
      state.hostName = hostInfo?.deviceName || '';
      setStatus(`${state.hostName || 'Uzak bilgisayar'} ile baglanti kuruluyor...`);
      setBadge('warn', 'Eslestiriliyor');
      await createViewerPeer();
    });

    state.socket.on('session:rejected', ({ reason }) => {
      showOverlay(reason || 'Baglanti reddedildi');
      setStatus(reason || 'Baglanti reddedildi');
      setBadge('danger', 'Reddedildi');
    });

    state.socket.on('session:ended', ({ reason }) => {
      showOverlay('Oturum sonlandirildi');
      setStatus(reason || 'Host oturumu sonlandirdi.');
      setBadge('danger', 'Oturum bitti');
      deactivateControl();

      if (state.peerConnection) {
        state.peerConnection.close();
        state.peerConnection = null;
      }
      state.dataChannel = null;
    });

    state.socket.on('signal:relay', async ({ from, payload }) => {
      try {
        if (state.hostSocketId && from !== state.hostSocketId) {
          return;
        }

        if (payload.description?.type === 'answer') {
          if (state.peerConnection?.signalingState === 'have-local-offer') {
            await state.peerConnection.setRemoteDescription(payload.description);
          }
          return;
        }

        if (payload.candidate && state.peerConnection) {
          await state.peerConnection.addIceCandidate(payload.candidate);
        }
      }
      catch (error) {
        setStatus(`Sinyal hatasi: ${error.message}`);
      }
    });
  }

  window.addEventListener('beforeunload', () => {
    if (state.socket && state.hostSocketId) {
      state.socket.emit('session:end', {
        to: state.hostSocketId,
        reason: 'Izleyici penceresi kapatildi.',
      });
    }

    if (state.thumbTimer) {
      clearInterval(state.thumbTimer);
    }

    if (state.peerConnection) {
      state.peerConnection.close();
    }
    if (state.socket) {
      state.socket.disconnect();
    }
  });

  installTopbarActions();
  installControlListeners();
  connect();
})();
